...
public class StreamingActivity extends Activity {
			...
			if (data instanceof DeviceSensorsAsyncData) {
				...
				if (data_list != null) {
					...
					for (DeviceSensorsData datum : data_list) {
						...
						if (attitude != null) {
							mImuView.setPitch(""
									+ attitude.getAttitudeSensor().pitch);
							mImuView.setRoll(""
									+ attitude.getAttitudeSensor().roll);
							mImuView.setYaw(""
									+ attitude.getAttitudeSensor().yaw);
						}

						// Show accelerometer data
						AccelerometerData accel = datum.getAccelerometerData();
						if (attitude != null) {
							double shakesThreshold = accel
									.getFilteredAcceleration().x
									+ accel.getFilteredAcceleration().y
									+ accel.getFilteredAcceleration().z;

							if (shakesThreshold > 4.0) {
								if (scoring.getTeamScoring() == 1) {

									mShakeFilteredView.setShakesCount(""
											+ scoring.incShakesCount());
								} else {
									mShakeFilteredView.setShakesCount2(""
											+ scoring.incShakesCount2());
								}
								// System.out.println("Shakes: "+shakesCount);
							}

							mAccelerometerFilteredView.setX(""
									+ accel.getFilteredAcceleration().x);
							mAccelerometerFilteredView.setY(""
									+ accel.getFilteredAcceleration().y);
							mAccelerometerFilteredView.setZ(""
									+ accel.getFilteredAcceleration().z);
						}
					}
				}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		startActivityForResult(new Intent(this, StartupActivity.class),
				sStartupActivity);
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK) {
			...
			if (requestCode == sStartupActivity) {
				...
				FrontLEDOutputCommand.sendCommand(mRobot, 00);

				// DeviceMessenger.getInstance().removeAsyncDataListener(robot,
				// dataListener)
				// RawMotorCommand.sendCommand(mRobot,
				// RawMotorCommand.MOTOR_MODE_FORWARD, 255,
				// RawMotorCommand.MOTOR_MODE_FORWARD, 255);
			}
		}
	}
